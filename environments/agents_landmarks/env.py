...
class agentslandmarks:
    ...
    def set_positions_idx(self):
        ...
        for _ in a:
            b2.append(_)
        a = [i for i in range(78,100)]
        for _ in a:
            b2.append(_)
        a = [30,31,40,41,50,51,60,61,70,71,28,29,38,39,48,49,58,59,68,69]
        for _ in a:
            b2.append(_)
        #print(b2)
        a1 = [44,53,56]
    ...
    def update_positions(self, pos_list, act_list):
        ...
        for idx in xrange(len(pos_list)):
            if act_list[idx] != 4:
                pos_act_applied = list(map(operator.add, pos_list[idx], self.A_DIFF[act_list[idx]])) #######################check
